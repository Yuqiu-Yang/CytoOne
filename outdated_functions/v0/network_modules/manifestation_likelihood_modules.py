...
class y_module(nn.Module):
    def __init__(self,
        ...
        if distribution_type == "ZILN":
            ...
            self.scale = nn.Parameter(mu_normal_scale, requires_grad=False)
            self.l = -F.softplus(nn.Parameter(torch.randn(1)))
        else:
            raise NotImplementedError
    
    def _update_mu_scale(self, mu_normal_scale):
        if self.distribution_type == "MU":
            self.scale = nn.Parameter(mu_normal_scale, requires_grad=False)
        else:
            raise Exception("Only when the distribution is MU can we update the scale\n")
    
    def forward(self, w, z, mu_normal_scale=None):
        ...
        if self.distribution_type == "MU":
            if mu_normal_scale is not None:
                self._update_mu_scale(mu_normal_scale=mu_normal_scale)
            return {
